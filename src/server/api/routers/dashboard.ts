...
import { courses, coursework, enrollments, submissions } from "@/lib/db/schema";
import { progressService } from "@/lib/services/progress";
import { protectedProcedure, router } from "../trpc";

// Helper functions from the REST API logic
...
async function getStudentDashboardData(userId: string) {
	...
	const upcomingDeadlines = await db.query.coursework.findMany({
		where: and(
			...
			sql`${coursework.dueDate} < NOW() + INTERVAL '14 days'`,
		),
		with: {
			course: {
				columns: { name: true },
	...
	return {
		...
		upcomingDeadlines: upcomingDeadlines.map((coursework) => ({
			...
			isSubmitted:
				coursework.submissions.length > 0 &&
				coursework.submissions[0].state === "TURNED_IN",
			isLate:
				coursework.submissions.length > 0 && coursework.submissions[0].late,
		})),
...
export const dashboardRouter = router({
	get: protectedProcedure.query(async ({ ctx }) => {
		...
		switch (userRole) {
			...
			case "coordinator":
				return await getCoordinatorDashboardData(userId);
			default:
